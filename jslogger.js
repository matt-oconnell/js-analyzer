...
function encodeObject(o) {
	if (_.isNumber(o)) {
		...
		if (!_.has(o, 'smallObjId_hidden_')) {
			...
			Object.defineProperty(o, 'smallObjId_hidden_', {
				value: smallObjId,
				enumerable: false
			});
			smallObjId++;
		...
		if (encodedHeapObjects[String(o.smallObjId_hidden_)] !== undefined) {
			...
			if (_.isFunction(o)) {
				...
				if (_.isObject(o.prototype)) {
					...
					if (!_.isEmpty(o.prototype) || (_.isObject(prototypeOfPrototype) && !_.isEmpty(prototypeOfPrototype))) {
        ...
        */
				...
				newEncodedObj.push('JS_FUNCTION', o.name, funcCodeString, /* code string*/ funcProperties.length ? funcProperties : null, /* OPTIONAL */ null /* parent frame */ );
			} else if (_.isArray(o)) {
...
function listener(event, execState, eventData, data) {
	...
	if (all_userscript_frames.length > 0) {
		...
		var curStack = all_userscript_frames.map(function(f) {
			return getCanonicalFrameId(f);
		});
		...
		for (i = 0; i < all_userscript_frames.length - 1; /* last frame is fake 'top-level' global frame */ i++) {
			...
			for (ii = f.scopeCount() - 1; ii >= 0; ii--) {
				...
				*/
				...
				if (scopeType === 1 || scopeType === 4) { // Local or Catch (for exceptions)
					...
					assert(scopeType === 0 || scopeType === 7);
				}
			}
			if (f.isAtReturn()) {
		...
		for (ii = topFrame.scopeCount() - 1; ii >= 0; ii--) {
			sc = topFrame.scope(ii);
			/* From v8/src/debug/debug-scopes.h
...
try {
	...
	var overrideScope = {
		setInterval: () => {
			throw 'Error: setInterval() is not supported by Python Tutor'
		},
		setTimeout: () => {
			throw 'Error: setTimeout() is not supported by Python Tutor'
		},
		setImmediate: () => {
			throw 'Error: setImmediate() is not supported by Python Tutor'
		},
	}
...
function finalize() {
	...
	var blob = {
		code: isTypescript ? originalTsCod : cod,
		trace: curTrace
	};
	if (argv.jsfile) {
